...
public abstract class ChunkMapMixin {
    ...
    private void modifyGenerator(Args args){
        ...
        if(context.generator() instanceof NoiseBasedChunkGenerator generator) {
            if (context.level().dimension() == Level.OVERWORLD && chunk.getPos().getMinBlockX() > 12550824) {
            ...
            WorldGenContext modifiedContext = new WorldGenContext(context.level(), generator, context.structureManager(), context.lightEngine(), context.mainThreadExecutor(), context.unsavedListener());
