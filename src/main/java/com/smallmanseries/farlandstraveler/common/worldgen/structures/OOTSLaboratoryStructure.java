    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (Config.GENERATE_OOTS_LABORATORY.getAsBoolean()) {
            return Optional.of(new Structure.GenerationStub(this.pos, builder -> {
                switch (this.part) {
                    case "exterior" ->
                            builder.addPiece(new Piece(context.structureTemplateManager(), EXTERIOR, new BlockPos(this.pos.getX() - 11, this.pos.getY(), this.pos.getZ() - 51)));
                    case "gate" ->
                            builder.addPiece(new Piece(context.structureTemplateManager(), GATE, new BlockPos(this.pos.getX() - 73, this.pos.getY() - 10, this.pos.getZ() - 33)));
                    case "maze" -> {
                    ...
                    case "cottage" ->
                            builder.addPiece(new Piece(context.structureTemplateManager(), COTTAGE, new BlockPos(this.pos.getX() - 82, this.pos.getY() - 20, this.pos.getZ() - 50)));
                    case "basement" -> {
