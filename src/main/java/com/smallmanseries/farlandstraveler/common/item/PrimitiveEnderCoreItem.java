...
public class PrimitiveEnderCoreItem extends Item {
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (Math.max(player.position().x(), player.position().z()) > (Config.FAR_LANDS_DISTANCE.getAsInt() - 4) || Math.min(player.position().x(), player.position().z()) < -Config.FAR_LANDS_DISTANCE.getAsInt()) {
            level.playSound(
                    null,
                    player.getX(),
                    player.getY(),
            ...
            if (player.hasEffect(MobEffects.LUCK)) {
                ...
                if (effect != null) {
                    effect.update(new MobEffectInstance(MobEffects.LUCK, effect.getDuration() + 200, 0, false, false, true));
                }
            } else {
                player.addEffect(new MobEffectInstance(MobEffects.LUCK, 200, 0, false, false, true));
            }
        }
